## Prompt-based Inference with llama.cpp

[`llama-cpp-inference.py`](./llama-cpp-inference.py) runs prompt-based inference with a local GGUF model through `llama-cpp-python` (for example `gpt-oss-120B`).
...
- `MODEL_NAME`: model name used in output columns and filenames
- `MODEL_PATH`: path to the local `.gguf` model
...
- `N_GPU_LAYERS`, `N_CTX`, `N_THREADS`, `N_BATCH`, `USE_MLOCK`, `USE_MMAP`: llama.cpp runtime settings
- `STOP_SEQUENCES`: stop strings for generation
...
The script loads a local GGUF model with `Llama(...)` from `llama_cpp`.

### Output

The script writes cleaned model responses to:

- `model_response_{MODEL_NAME}`
- `model_response_{MODEL_NAME}_subtype`

For gpt-oss-style outputs, the script strips wrapper artifacts and removes text after configured stop sequences before saving.
...
and access to the local GGUF model file specified in `MODEL_PATH`.